...
# Store tokens securely for session
if openai_key:
...
# Validation
# -------------------------------------------------
if uploaded_file and (not openai_key or not hf_token):
    st.warning("⚠️ Please enter both OpenAI and Hugging Face tokens in the sidebar.")

# -------------------------------------------------
# Processing
# -------------------------------------------------
if uploaded_file and openai_key and hf_token:
    with tempfile.TemporaryDirectory() as temp_dir:
        input_audio = os.path.join(temp_dir, uploaded_file.name)
        clean_audio = os.path.join(temp_dir, "clean.wav")
        ...
        if st.button("🚀 Process Meeting"):
            ...
            with colA:
                st.subheader("🗣️ Speaker-wise Transcript")
                st.text_area("Transcript", final_text, height=450)
